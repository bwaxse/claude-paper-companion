...
class ZoteroService:
    ...
    def __init__(self, api_key: Optional[str] = None, library_id: Optional[str] = None, library_type: str = "user"):
        ...
        Args:
            api_key: Zotero API key (uses settings if not provided)
            library_id: Zotero library ID (uses settings if not provided)
    ...
    async def search_papers(self, query: str, limit: int = 10) -> List[ZoteroItemSummary]:
        ...
        if re.match(r'10\.\d+/.*', query):
            items = await loop.run_in_executor(None, partial(self.zot.items, q=query, limit=limit))
        else:
            # General search
            items = await loop.run_in_executor(None, partial(self.zot.items, q=query, limit=limit))
    ...
    async def list_recent(self, limit: int = 20) -> List[ZoteroItemSummary]:
        ...
        # Get recent items sorted by date added
        items = await loop.run_in_executor(
            None,
            partial(self.zot.items, limit=limit, sort='dateAdded', direction='desc')
        )
...
def get_zotero_service() -> ZoteroService:
    """
    Get global Zotero service instance (singleton pattern).

    Returns:
        ZoteroService: Zotero service instance
    """
...
async def search_papers(query: str, limit: int = 10) -> List[ZoteroItemSummary]:
    """Search Zotero library."""
    service = get_zotero_service()
    return await service.search_papers(query, limit)


async def get_paper_by_key(key: str) -> Optional[ZoteroItem]:
    """Get paper by Zotero key."""
    service = get_zotero_service()
    return await service.get_paper_by_key(key)


async def list_recent(limit: int = 20) -> List[ZoteroItemSummary]:
    """List recent papers."""
    service = get_zotero_service()
