...
class TestTokenUsage:
    ...
    def test_cost_calculation(self):
        ...
        usage.output_tokens = 500_000   # 500K tokens

        # Haiku cost: 1M * 0.80 + 500K * 4.00 = $0.80 + $2.00 = $2.80
        haiku_cost = usage.calculate_cost(MODELS["haiku"])
        assert haiku_cost == pytest.approx(2.80, rel=0.01)

        # Sonnet cost: 1M * 3.00 + 500K * 15.00 = $3.00 + $7.50 = $10.50
        sonnet_cost = usage.calculate_cost(MODELS["sonnet"])
        assert sonnet_cost == pytest.approx(10.50, rel=0.01)
...
class TestClaudeClient:
    ...
    async def test_initial_analysis(self, claude_client):
        ...
        result, usage = await claude_client.initial_analysis(
            ...
            pdf_text="Test paper content",
        )

        # Verify result
        assert result == "Test analysis result"
        assert usage["model"] == MODELS["haiku"]
